...
class Main:
    ...
    def __post_init__(self) -> None:
        if not issubclass(type(self.__cipher), ICipher):
            raise InvalidCipherError("Invalid cipher")
...
if __name__ == "__main__":
    ...
    try:
        ...
        decrypter = CesarCipherDecrypter(cipher.encrypt, conversor)

        program = Main(cipher=cipher, decrypter=decrypter)
        print(program.encrypt())
        print(program.decrypt())
    except (InvalidCharacterError, 
            InvalidDecrypterError, 
            InvalidCipherError) as err:
        print(err)
